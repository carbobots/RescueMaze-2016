...
void Frente() {
    ...
    double correcao = (lendoMpuGyro() * -1) - (anguloInicial * -1);

    MostrarLCD(CIMA, 6, String(correcao));
    CurvaGyro(correcao - 2, forca);
    pararMotores();
...
void Subida() {
  ...
  while(lendoMpuAccel() < angulo_rampa_subida) { // enquanto for menor que o angulo, ele só vai andando
...
void Algoritmo() {
  ...
  MostrarLCD(BAIXO, 6, String(verificarNumeroParedes()));
            ...
            if(sistemaFechado() == true) {
               ...
               Frente();
                ...
                if (norte == 1) {
                  ...
                  if(verificarParede(LESTE) == false && verificarCordBlackSpace(xatual + 1, yatual) == false && verificarHistorico(xatual + 1, yatual) ==  false) {
                    Direita();
                    norte = 0; leste = 1; sul = 0; oeste = 0;
                  }
                  else if(verificarParede(NORTE) == false && verificarCordBlackSpace(xatual, yatual + 1) == false && verificarHistorico(xatual, yatual + 1) ==  false) {
                    Frente();
