...
/*
  FUNÃ‡Ã•ES DE VERIFICAÃ‡ÃƒO DO ROBÃ”
*/
...
bool verificarBlackSpaceAoRedor( int ladoblack) {

  if(ladoblack == quadradonegro) {
    return true;
  } else {
    return false;
  }
...
bool sistemaFechado() {
  ...
  return true;
  } else if(quadradonegro == 1 && lerSharp(4) <= distancia_parede && lerSharp(2) <= distancia_parede) { // se tiver black space frente, parede direita e esquerda
    return true;
  } else if (quadradonegro == 2 && lerSharp(4) <= distancia_parede && lerSharp(1) <= distancia_parede) { // se tiver black space frente, parede direita e esquerda
    return true;
  } else if (quadradonegro == 3 && lerSharp(4) <= distancia_parede && lerSharp(2) <= distancia_parede) { // se tiver black space frente, parede direita e esquerda
    return true;
  }  else if(quadradonegro == 4 && lerSharp(1) <= distancia_parede && lerSharp(2) <= distancia_parede) { // se tiver black space frente, parede direita e esquerda
    return true;
  }    else {
...
String checarBytesQuadrado() {
    ...
    if(lendoMpuAccel() < angulo_rampa_subida || lendoMpuAccel() > angulo_rampa_descida) {
      k = 1;
    }
      String result;
      result += a;
